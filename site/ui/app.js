...
function onDocumentClick(event){if(event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;const link=event.target.closest('a[data-ui-route]');if(!link)return;const url=new URL(link.href,location.href);if(url.origin!==location.origin)return;event.preventDefault();void navigate(link.dataset.uiRoute||routeKeyForPath(url.pathname))}
...
async function boot(){
  ...
  profile=state.profile;window.__SINDHORN_AUTH_PROFILE__=profile;renderHeader();renderFooter();
  document.addEventListener('click',onDocumentClick);addEventListener('popstate',()=>void mountRoute(routeKeyForPath(location.pathname),{animate:false}));
