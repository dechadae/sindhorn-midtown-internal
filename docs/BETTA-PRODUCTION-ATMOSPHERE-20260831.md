...
Each period may carry a style (r29a): the output of `site/betta-random.js`, a verbatim port of the Mac Betta Metal Lab randomizer (SplitMix64; colours and fin form only; camera and composition are never randomized). Styles are chosen and judged on the Readability Test (`#readability`, developer account only) and are configuration: they live in `public.sindhorn_betta_periods` (r29b; read through the anon-readable `sindhorn_betta_periods_v1()`, written through `sindhorn_betta_period_save_v1` / `_reset_v1` under `system.manage`, the 4.5:1 contract re-checked server-side), the runtime boots from this device's copy (`localStorage`) and the shell refreshes that copy from the server after the first frame; `setBettaStyle` / `setBettaStyles` / `saveBettaStyles` are the only writers in the runtime. A period without a style renders its bundled baseline.
