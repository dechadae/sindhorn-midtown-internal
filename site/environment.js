...
void main(){
  ...
  vec2 sd=uv-uSun;sd.x*=aspect;float sunDist=length(sd);float sunAbove=smoothstep(-4.0,1.0,uSolarAltitude);float clearSun=sunAbove*(1.0-cloud*.94);float diffuseSun=sunAbove*(.18+(1.0-cloud)*.82);float sunCoreGlow=exp(-sunDist*sunDist*1800.0);float sunHalo=exp(-sunDist*sunDist*180.0);float sunAura=exp(-sunDist*sunDist*38.0);vec3 sunWarm=mix(vec3(1.0,.68,.36),vec3(1.0,.98,.86),smoothstep(5.0,40.0,uSolarAltitude));c+=sunWarm*(sunCoreGlow*.68*clearSun+sunHalo*.18*diffuseSun+sunAura*.035*diffuseSun);
