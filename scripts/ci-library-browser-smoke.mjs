...
async function developerViewport(browser,width,height){
  ...
  try{
    ...
    await page.locator('[data-ci-open-dialog]').evaluate(node=>node.click());await page.waitForFunction(()=>document.querySelector('[data-ci-dialog]')?.open===true);await page.locator('[data-ci-dialog-done]').evaluate(node=>node.click());await page.waitForFunction(()=>document.querySelector('[data-ci-dialog]')?.open===false);
...
async function employeeGate(browser){
  ...
  try{
    ...
    await page.locator('#app-footer [data-settings-section-nav="account"]').evaluate(node=>node.click());
    await page.waitForFunction(()=>document.querySelector('#app-footer [data-settings-section-nav="account"][aria-current]')&&document.querySelectorAll('.settings-account-section').length===1,{timeout:25000});
    const accountRestored=await page.evaluate(()=>({count:document.querySelectorAll('.settings-account-section').length,current:document.querySelector('#app-footer [data-settings-section-nav][aria-current]')?.dataset.settingsSectionNav||null}));assert(accountRestored.count===1&&accountRestored.current==='account',`Account did not restore after blank System ${JSON.stringify(accountRestored)}`);
