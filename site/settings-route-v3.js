...
// Settings reuses the proven F&B select component; load its core + interaction styling only when Settings is mounted.
...
export async function mountSettingsRoute(root){
  ...
  try{
    await Promise.all([
      ...
      ensureStyle('link[data-business-card-settings-style]','/business-card-settings.css?v=11','data-business-card-settings-style'),
