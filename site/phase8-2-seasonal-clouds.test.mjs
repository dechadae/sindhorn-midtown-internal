...
// Cloud morphology contract: mid/low clouds are broad anisotropic tropical banks.
// Higher-frequency random texture may roughen edges and interior lighting, but regular
// circle/lobe lattices are forbidden because they create visible repeating scallops.
for(const token of ['midBankField','midBankCore','midTexture','midShapeField','midEdgeBreakup','lowBankField','lowBankCore','lowTexture','lowShapeField','lowEdgeBreakup'])assert.ok(ATMOSPHERE_FRAGMENT_SHADER.includes(token),`${token} morphology stage must remain explicit`);
assert.ok(ATMOSPHERE_FRAGMENT_SHADER.includes('vec2(.64,1.18)'),'mid-cloud bank sampling must stay strongly wider than tall');
assert.ok(ATMOSPHERE_FRAGMENT_SHADER.includes('vec2(.55,1.12+.10*lowBuildRound)'),'low monsoon bank must stay broad while convective build affects crown texture');
...
assert.ok(ATMOSPHERE_FRAGMENT_SHADER.includes('mWarp=')&&ATMOSPHERE_FRAGMENT_SHADER.includes('lWarp='),'mid/low detail fields must keep cheap asymmetric domain warping');
for(const forbidden of ['midCells','lowCells','mLobeA','mLobeB','lLobeA','lLobeB','mCellA','mCellB','lCellA','lCellB'])assert.ok(!ATMOSPHERE_FRAGMENT_SHADER.includes(forbidden),`${forbidden} regular lobe lattice must stay retired`);
assert.ok(!ATMOSPHERE_FRAGMENT_SHADER.includes('fract(vec2(mp.x*22.0'),'mid-cloud regular cell grid must stay retired');
assert.ok(!ATMOSPHERE_FRAGMENT_SHADER.includes('fract(vec2(lp.x*22.0'),'low-cloud regular cell grid must stay retired');
...
console.log(`Phase 8.2 seasonal/cloud fixtures PASS (${PHASE82_FIXTURE_KEYS.length} deterministic cases; ${textureSamples} bounded noise call sites; per-launch cloud seed; broad anisotropic banks + stochastic edge breakup; no regular lobe lattice)`);
