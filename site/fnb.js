...
export async function mountFnbRoute(root,{profile}={}){
  ...
  function activationHTML(activation){const total=activation.artworks.length,done=activation.artworks.filter(item=>isDone(item.id)).length,complete=total>0&&done===total;return`<article class="fnb-art-card${complete?' is-complete':''}"><button class="fnb-art-head" type="button" data-art-toggle aria-expanded="false"><span class="fnb-art-head-text"><strong>${esc(activation.outlet)}</strong><span class="fnb-art-meta">${esc(activation.time)} · IHG One Rewards ${esc(activation.discount)}</span></span><span class="fnb-art-tally">${done}/${total}<i>${complete?'✓':''}</i></span><span class="fnb-art-chevron" aria-hidden="true"></span></button><div class="fnb-task-list" hidden>${taskRows(activation)}</div></article>`}
