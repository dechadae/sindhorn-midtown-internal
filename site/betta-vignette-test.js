...
descriptionNode.textContent='Final Betta materials with independent XYZ poses, deliberately asymmetric editorial crops, negative space and calibrated device-tilt parallax.';
...
function paint(){
  ...
  stateNode.textContent=`${day.periodName||day.targetPeriodKey||'—'} · Fish #${current?.referenceId||'—'} · editorial crop + tilt · ${Math.round((Number(day.transitionMix)||0)*100)}%`;
...
const {initEnvironment}=await import('./betta-environment.js?v=editorial-crop-1');
