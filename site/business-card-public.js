...
function render(){
  ...
  let qr='';try{qr=qrSvg(url,{foreground:'#17131F',background:'#FFFFFF',quiet:4}).replace('One-time sign-in QR code','Business card QR code')}catch(_){qr='<span class="public-card-qr-error">QR unavailable</span>'}
  root.innerHTML=`<section class="public-card-panel" aria-labelledby="publicCardName"><header class="public-card-head"><p class="public-card-kicker">Digital business card</p><h1 id="publicCardName">${esc(card.displayName).toUpperCase()}</h1>${card.positionTitle?`<p class="public-card-title">${esc(card.positionTitle)}</p>`:''}<p class="public-card-hotel">${esc(card.hotelName||HOTEL_NAME)}</p></header><div class="public-card-qr" data-card-qr>${qr}</div><div class="public-card-logo-wrap"><img class="public-card-logo" src="${esc(logo)}" alt="${esc(card.hotelName||HOTEL_NAME)}"></div><div class="public-card-details">${detail('Work email',card.workEmail,email)}${detail('Business mobile',card.businessMobile,telHref(card.businessMobile))}${detail('Direct phone',card.directPhone,telHref(card.directPhone))}${detail('Hotel telephone',card.hotelMainPhone,telHref(card.hotelMainPhone))}${detail('Hotel address',card.hotelAddress)}${detail('Hotel website',card.hotelWebsite,card.hotelWebsite)}</div><div class="public-card-actions"><a class="public-card-action is-primary" href="${esc(vcfUrl)}" data-add-contact>Add to contacts</a>${call?'<a class="public-card-action" data-call>Call</a>':''}${email?'<a class="public-card-action" data-email>Email</a>':''}<button class="public-card-action" type="button" data-share-card>Share</button></div><p class="public-card-status" data-card-status role="status" aria-live="polite"></p><footer><span>${esc(url.replace(/^https?:\/\//,''))}</span></footer></section>`;
  const callNode=root.querySelector('[data-call]');if(callNode)callNode.href=call;const emailNode=root.querySelector('[data-email]');if(emailNode)emailNode.href=email;
}
async function shareCard(){const url=businessCardUrl(location.origin,card.slug),title=`${card.displayName} | ${card.hotelName}`,text=[card.displayName,card.positionTitle,card.hotelName].filter(Boolean).join(' · ');if(typeof navigator.share==='function'){try{await navigator.share({title,text,url});return}catch(error){if(error?.name==='AbortError')return}}showStatus(await copyText(url)?'Link copied':'Copy link failed')}
