...
async function render(){
  ...
  if(!prepared){writeButton('Preparing alerts…',true);writeStatus('Preparing secure notification support.','กำลังเตรียมระบบแจ้งเตือนที่ปลอดภัย','working');try{await prepare()}catch(error){console.warn('Push preparation failed',error);prepared=false;registration=null;publicKey='';subscription=null;writeButton('Retry alerts',false);writeStatus('Alert setup did not finish. Tap Retry alerts to try again.','การเตรียมระบบแจ้งเตือนยังไม่สำเร็จ แตะ Retry alerts เพื่อลองอีกครั้ง','error');return}}
