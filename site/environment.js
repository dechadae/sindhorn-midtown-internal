...
void main(){
  ...
  vec2 sd=uv-uSun;sd.x*=aspect;float sunDist=length(sd);float sunAbove=smoothstep(-4.0,1.0,uSolarAltitude);float clearSun=sunAbove*(1.0-cloud*.92);float sunHalo=exp(-sunDist*18.0)*clearSun;float sunR=.020;float sunRim=smoothstep(sunR*.66,sunR*.88,sunDist)*(1.0-smoothstep(sunR*.92,sunR*1.06,sunDist));float sunCorona=exp(-abs(sunDist-sunR)*115.0);vec3 sunWarm=mix(vec3(1.0,.64,.34),vec3(1.0,.98,.84),smoothstep(5.0,40.0,uSolarAltitude));c+=sunWarm*(sunHalo*.18+sunRim*1.12+sunCorona*.28)*clearSun;
  vec2 md=uv-uMoon;md.x*=aspect;float moonDist=length(md);float moonAbove=smoothstep(-4.0,1.0,uMoonAltitude);float clearMoon=moonAbove*(1.0-cloud*.94);float moonR=.020;float moonRim=smoothstep(moonR*.64,moonR*.88,moonDist)*(1.0-smoothstep(moonR*.92,moonR*1.07,moonDist));float moonHalo=exp(-moonDist*20.0);vec3 moonCool=vec3(.82,.90,1.0);c+=moonCool*(moonRim*.92+moonHalo*.11*uMoonIllumination)*clearMoon;
