...
function codeDetails(code,label='Canonical code'){
  return`<details class="ci-code-disclosure"><summary>${esc(label)}</summary><div class="ci-code-wrap"><button class="ci-copy-code app-control" type="button" data-ci-copy>Copy</button><pre><code>${esc(code)}</code></pre></div></details>`;
}
...
function tokenMarkup(){return UI_SYSTEM_TOKENS.map(token=>`<article class="ci-token app-card" data-kind="${esc(token.kind)}" data-ci-token="${esc(token.name)}"><div class="ci-token-swatch" data-ci-token-swatch></div><strong>${esc(token.label)}</strong><code>${esc(token.name)}</code><small data-ci-token-value>${esc(token.fallback)}</small></article>`).join('')}
function componentDocs(){return UI_SYSTEM_COMPONENTS.map(item=>`<article class="ci-doc-card app-card"><div class="ci-doc-label"><span>${esc(item.label)}</span><span>${esc(item.owner.split(' + ')[0])}</span></div><h3>${esc(item.label)}</h3><code class="ci-doc-selector">${esc(item.selector)}</code><div class="ci-doc-meta"><div><span>Use</span><b>${esc(item.use)}</b></div><div><span>Avoid</span><b>${esc(item.avoid)}</b></div><div><span>A11y</span><b>${esc(item.a11y)}</b></div><div><span>Owner</span><b>${esc(item.owner)}</b></div></div></article>`).join('')}
function rulesMarkup(){return UI_SYSTEM_RULES.map(([rule,why])=>`<article class="ci-rule app-card"><i>◇</i><div><strong>${esc(rule)}</strong><p>${esc(why)}</p></div></article>`).join('')}
...
function surfaceSection(){return section('surfaces','Cards Communicate Role','A surface looks actionable only when it is actionable. F&B remains the authority for the app’s compact glass card; informational surfaces keep the same material without a press promise.',`
  <div class="ci-grid">
    <div class="ci-specimen"><div class="ci-specimen-label"><span>Actionable card</span><span>F&B authority</span></div><div class="ci-specimen-stage"><article class="app-action-card"><button class="app-action-card-button" type="button" data-ci-demo-action><div class="app-action-card-status"><span class="fnb-text-label">Upcoming</span><span class="app-action-card-relative">Starts soon</span></div><h3 class="app-action-card-title">Actionable Card</h3><p class="app-action-card-outlets">Whole surface opens a detail or route.</p><div class="app-action-card-foot"><span>Open detail</span><span class="fnb-chevron">›</span></div></button></article></div>${codeDetails(COMPONENT_CODE.card)}</div>
...
function disclosureSection(){return section('disclosures','Expandable Content Has One Rhythm','A disclosure keeps the full content in the DOM, exposes aria-expanded, rotates the same chevron language and uses the established 420ms rhythm.',`
  <div class="ci-specimen ci-disclosure-demo"><div class="ci-specimen-label"><span>Room-style disclosure</span><span>Factsheet component</span></div><article class="app-disclosure" data-ci-disclosure><button class="app-disclosure-button" type="button" aria-expanded="false" data-ci-disclosure-button><span class="app-disclosure-copy"><span class="factsheet-room-index">01 · Example</span><strong>Expandable Reference</strong><span class="factsheet-room-bed">Tap to reveal structured detail</span></span><svg class="factsheet-room-chevron" viewBox="0 0 20 20" aria-hidden="true"><path d="M7 5l5 5-5 5"/></svg></button><div class="app-disclosure-panel"><div class="app-disclosure-panel-inner"><div class="factsheet-room-context"><span>Details</span><ul><li>Content stays in the same card.</li><li>The control owns aria-expanded.</li><li>Reduced motion removes transition without hiding state.</li></ul></div></div></div></article>${codeDetails(COMPONENT_CODE.disclosure)}</div>`)}
function formSection(){return section('forms','Forms Stay Quiet and Explicit','Labels remain visible, focus uses Sorbet, and validation augments rather than replaces the field. Placeholder text is supporting copy—not the label.',`
  <div class="ci-specimen"><div class="ci-specimen-label"><span>Fields</span><span>Settings authority</span></div><div class="ci-form-demo"><div class="app-field"><label for="ciName">Display name</label><input id="ciName" type="text" value="Sindhorn Midtown"></div><div class="app-field"><label for="ciEmail">Work email</label><input id="ciEmail" type="email" value="name@ihg.com"></div><div class="app-field"><label for="ciError">Validation state</label><input id="ciError" type="text" value="Needs attention" aria-invalid="true" aria-describedby="ciErrorText"><span class="ci-inline-error" id="ciErrorText">Explain what the user can correct.</span></div><div class="app-field"><label for="ciDisabled">Disabled</label><input id="ciDisabled" type="text" value="Unavailable" disabled></div></div>${codeDetails(COMPONENT_CODE.input)}</div>`)}
...
function imagerySection(){const image=HOTEL_FACTSHEET_IMAGES.overview;return section('imagery','Images Support the Reference','Use official hotel imagery where content requires it, preserve an intentional aspect ratio, provide useful alt text and label external source provenance when relevant.',`
  <div class="ci-grid"><figure class="ci-image-demo app-card"><img src="${esc(image.src)}" alt="${esc(image.alt)}" loading="lazy"><figcaption>Official hotel image · cover crop</figcaption></figure><div class="ci-specimen"><div class="ci-specimen-label"><span>Rules</span><span>Content imagery</span></div><dl class="ci-fact-list"><div><dt>Fit</dt><dd>Use object-fit: cover for editorial crops; contain only when the entire object must remain visible.</dd></div><div><dt>Radius</dt><dd>Match the 14px card/image surface family unless the component owner says otherwise.</dd></div><div><dt>Alt</dt><dd>Describe the information the image contributes. Decorative images use empty alt text.</dd></div><div><dt>Loading</dt><dd>Lazy-load below-the-fold imagery; do not lazy-load the primary above-the-fold identity asset.</dd></div></dl>${codeDetails(COMPONENT_CODE.image)}</div></div>`)}
function navigationSection(){return section('navigation','Navigation Geometry Never Depends on Permission','The persistent footer owns global routes. Settings always shows Account / People / Comms / System; capabilities decide what those sections render, not whether the four-tab rail changes shape.',`
  <div class="ci-grid"><div class="ci-specimen"><div class="ci-specimen-label"><span>Global footer</span><span>Live below this page</span></div><dl class="ci-fact-list" data-ci-footer-facts></dl><p class="ci-note">The actual persistent footer at the bottom of the viewport is the live specimen. CI does not draw a second imitation.</p></div><div class="ci-specimen"><div class="ci-specimen-label"><span>Settings rail</span><span>Fixed invariant</span></div><div class="ci-grid-2"><span class="ci-state-card app-card"><strong>Account</strong></span><span class="ci-state-card app-card"><strong>People</strong></span><span class="ci-state-card app-card"><strong>Comms</strong></span><span class="ci-state-card app-card"><strong>System</strong></span></div><p class="ci-note">Unauthorized sections are blank. The four-tab navigation remains stable.</p></div></div>${codeDetails(COMPONENT_CODE.navigation,'Authenticated route link')}`)}
function stateSection(){return section('states','Operational States Stay Calm','Loading, empty, success, error, offline and disabled states should explain what happened and what the employee can do next without changing the product’s visual language.',`
  <div class="ci-state-grid"><article class="ci-state-card app-card"><span>Loading</span><strong>Loading employees…</strong><p>Reserve final geometry where possible; avoid late layout jumps.</p></article><article class="ci-state-card app-card"><span>Empty</span><strong>No messages yet</strong><p>State the absence plainly. Do not manufacture decorative content.</p></article><article class="ci-state-card app-card" data-tone="success"><span>Success</span><strong>Saved</strong><p>Confirm the meaningful result, then return control to the user.</p></article><article class="ci-state-card app-card" data-tone="error"><span>Error</span><strong>Could not save</strong><p>Explain the recoverable action instead of exposing transport internals.</p></article></div>`)}
function motionSection(){return section('motion','Motion Explains State','Press feedback is immediate, component state settles with one easing family, disclosures are deliberate, and route navigation remains the separate persistent-shell crossfade.',`
  <div class="ci-motion-demo"><div class="ci-motion-step app-card"><b>160</b><span>Press ms</span></div><div class="ci-motion-step app-card"><b>260</b><span>State ms</span></div><div class="ci-motion-step app-card"><b>420</b><span>Disclosure ms</span></div><div class="ci-motion-step app-card"><b>280</b><span>Route ms</span></div></div><div class="ci-specimen" style="margin-top:9px"><div class="ci-specimen-label"><span>Accessibility</span><span data-ci-motion-preference>Motion preference</span></div><dl class="ci-fact-list"><div><dt>Focus</dt><dd>Every actionable keyboard target exposes a visible focus state.</dd></div><div><dt>Touch</dt><dd>Targets keep practical mobile hit areas even when the visual icon is compact.</dd></div><div><dt>Reduced</dt><dd>Suppress transforms/transitions while preserving visible state changes.</dd></div><div><dt>Semantics</dt><dd>Use buttons for actions, links for navigation, and native dialog/table semantics where available.</dd></div></dl></div>`)}
...
function renderPage(authority){
  ...
  return`<section class="ci-route" aria-labelledby="ciTitle">
    ...
    <div class="ci-status app-card" data-ci-status><div class="ci-status-head"><div class="ci-status-title"><i class="ci-status-dot"></i><strong data-ci-status-title>Checking live system…</strong></div><span class="ci-status-count" data-ci-status-count></span></div><div class="ci-check-list" data-ci-check-list></div></div>
    <nav class="ci-index" aria-label="UI Library sections">${UI_SYSTEM_SECTIONS.map(item=>`<button class="app-control" type="button" data-ci-jump="${esc(item.id)}">${String(item.index).padStart(2,'0')} ${esc(item.label)}</button>`).join('')}</nav>
    ${section('identity','The App Identity','This library documents the Sindhorn Midtown Internal product identity. The persistent hotel masthead above is the actual production header; routes never duplicate it.',`<div class="ci-grid"><div class="ci-identity-lockup app-card"><img src="/assets/brand/sindhorn-midtown-vignette-white.png" alt="Sindhorn Midtown, Vignette Collection"></div><div class="ci-specimen"><div class="ci-specimen-label"><span>Identity contract</span><span>Persistent shell</span></div><dl class="ci-fact-list"><div><dt>Product</dt><dd>Sindhorn Midtown Internal</dd></div><div><dt>Hotel</dt><dd>Sindhorn Midtown Hotel Bangkok, Vignette Collection by IHG</dd></div><div><dt>Account</dt><dd>${esc(profile.displayName||profile.employeeNumber||'Developer')}</dd></div><div><dt>Header</dt><dd>Mounted once by the authenticated shell</dd></div></dl></div></div>`)}
    ...
    <dialog class="app-dialog" data-ci-dialog><div class="app-dialog-body"><div class="app-dialog-head"><div><p class="app-dialog-kicker">UI Library</p><h2>Dialog Specimen</h2></div><button class="settings-close" type="button" data-ci-dialog-close aria-label="Close">×</button></div><p class="settings-support">This is the same dialog surface and controller used by Settings. New features should consume the owner instead of building another modal.</p><div class="app-dialog-actions"><button class="settings-primary" type="button" data-ci-dialog-done>Done</button></div></div></dialog>
  </section>`;
}
