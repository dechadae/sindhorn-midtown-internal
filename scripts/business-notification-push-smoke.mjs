...
try{
  ...
  assert(registration.permission==='granted',`Notification permission not granted ${JSON.stringify(registration)}`);
  ...
  try{registrationId=await Promise.race([registrationPromise,new Promise((_,reject)=>setTimeout(()=>reject(new Error('service worker registration id timeout')),12000))])}
    ...
    // Enabling the CDP domain after an already-known registration is also valid; toggling it makes
    // Chromium re-emit the registration inventory without changing the service worker itself.
  ...
  const state=await page.evaluate(async expected=>{
    ...
    const notifications=await registration.getNotifications({tag:expected.tag});
    return{
  ...
  console.log(JSON.stringify({ok:true,baseUrl:BASE_URL,registrationId,state}));
