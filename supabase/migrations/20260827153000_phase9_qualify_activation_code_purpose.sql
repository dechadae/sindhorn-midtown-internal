-- Phase 9: fix PL/pgSQL output-column ambiguity in manual activation.
-- The function returns a column named `purpose`, so every activation_codes
-- reference to the table column must be schema/table-qualified at runtime.
...
begin
  ...
  select ac.* into a
  from sindhorn_private.activation_codes ac
  where ac.employee_id = e.id
    and ac.purpose = v_expected_purpose
  ...
  if v_stored_hash is null or extensions.crypt(p_plain_code, v_stored_hash) <> v_stored_hash then
    ...
    update sindhorn_private.activation_codes ac
...
exception
  when others then
    raise log 'sindhorn_manual_activate internal error [%] %', sqlstate, sqlerrm;
    raise exception 'activation unavailable';
end
$function$;
