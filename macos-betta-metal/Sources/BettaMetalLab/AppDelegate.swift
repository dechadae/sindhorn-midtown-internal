...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let initialFrame = screen?.visibleFrame.insetBy(dx: (screen?.visibleFrame.width ?? 1200) * 0.08, dy: (screen?.visibleFrame.height ?? 800) * 0.08)
        ...
        metalView = MTKView(frame: window.contentView?.bounds ?? initialFrame)
    ...
    @objc private func selectFish(_ sender: NSMenuItem) { renderer.setManualPreset(sender.tag) }
    ...
    @objc private func nextFish(_ sender: Any?) { renderer.cycle(1) }
    @objc private func previousFish(_ sender: Any?) { renderer.cycle(-1) }
    @objc private func toggleDesktop(_ sender: Any?) { window.setDesktopMode(!window.desktopMode) }

    private func applyLaunchArguments() {
        let args = CommandLine.arguments
        if args.contains("--preview") { renderer.usePreviewMode() }
        if let fishArg = args.first(where: { $0.hasPrefix("--fish=") }), let value = Int(fishArg.split(separator: "=").last ?? ""), (1...8).contains(value) { renderer.setManualPreset(value - 1) }
        if args.contains("--desktop") { window.setDesktopMode(true) }
    }
    ...
    private func buildMenu() {
        ...
        for (index, preset) in BettaPreset.all.enumerated() {
            let item = NSMenuItem(title: "Fish #\(preset.referenceId) — \(preset.name)", action: #selector(selectFish(_:)), keyEquivalent: String(index + 1)); item.target = self; item.tag = index; bettaMenu.addItem(item)
        }
        bettaMenu.addItem(.separator())
        ...
        bettaMenu.addItem(.separator())
        let desktop = NSMenuItem(title: "Toggle Live Desktop", action: #selector(toggleDesktop(_:)), keyEquivalent: "d"); desktop.target = self; bettaMenu.addItem(desktop)
        NSApp.mainMenu = main
    }
