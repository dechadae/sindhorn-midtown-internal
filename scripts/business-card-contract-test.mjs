...
for(const required of ['.settings-modal-surface:not(.public-card-panel)','padding:0 22px 22px','padding:0 18px 18px','flex:1 1 0!important','align-items:stretch!important','flex-wrap:nowrap!important'])if(!dialogCss.includes(required))throw new Error(`Business Card parity contract missing ${required}`);
...
if(!settingsWrapper.includes('/settings-dialog-standard.css?v=1&r=6')||!settingsWrapper.includes('/business-card-component.css?v=1&r=3'))throw new Error('Central Settings dialog/card revised styles not loaded');
if(!routeRegistry.includes('settings-route-v3.js?v=10&r=6&d=5'))throw new Error('Settings route cache version missing');

console.log(JSON.stringify({slug:'dechak',url:`${ORIGIN}/dechak`,vcardUrl:`${ORIGIN}/dechak.vcf`,hotel:HOTEL,dialogStandard:{controller:'settings-dialog-standard.js',structure:'settings-modal-root > settings-modal-surface > settings-modal-scroll',open:300,close:180,scrollbar:'business-card-edge',bottomActions:'equal-flex'},cardStandard:{renderer:'business-card-renderer.js',component:'business-card-component.css',publicWebGL:false,presentIframe:false,fixedShell:true},ui:{editEmployee:'central-modal-shell',editCard:'central-modal-shell',presentQr:'central-modal-shell',publicCard:'same-card-static-shell'}},null,2));