...
function patchUpdated(){const route=document.querySelector('.fnb-route'),title=route?.querySelector('.fnb-detail-title')?.textContent?.trim();if(!route||!title)return;const item=FNB_PROMOTIONS.find(p=>p.title===title),updated=item?.updatedAt;if(!updated)return;for(const fact of route.querySelectorAll('.fnb-fact')){if(fact.querySelector('span')?.textContent?.trim()==='Updated'){const b=fact.querySelector('b');if(!b||b.dataset.fnbUpdated===updated)continue;const d=new Date(updated);b.textContent=Number.isNaN(d.valueOf())?String(updated).slice(0,10):new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Bangkok',day:'numeric',month:'long',year:'numeric'}).format(d);b.dataset.fnbUpdated=updated}}}
