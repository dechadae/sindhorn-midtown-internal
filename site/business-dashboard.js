...
async function refreshRoute(host,{force=false}={}){if(!host?.isConnected)return;host.setAttribute('aria-busy','true');try{if(force)clearBusinessDashboardCache();const data=await loadBusinessDashboard({force});if(!host.isConnected)return;host.innerHTML=render(data);host.dataset.businessDate=data.businessDate||'';document.dispatchEvent(new CustomEvent('sindhorn:business-dashboard-loaded',{detail:{businessDate:data.businessDate,revision:data.revision}}))}catch(error){if(host.isConnected)host.innerHTML=errorMarkup(error?.message)}finally{host?.removeAttribute('aria-busy')}}
...
export async function mountBusinessDashboardRoute(host){
  ...
  const onClick=event=>{
    ...
    if(event.target.closest('[data-bd-retry]'))refreshRoute(route,{force:true});
  };
  route.addEventListener('click',onClick);
  await refreshRoute(route);
  activeController={refresh:()=>refreshRoute(route,{force:true})};
