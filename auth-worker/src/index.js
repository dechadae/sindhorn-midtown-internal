...
const ROLES=new Set(['employee','supervisor','manager','admin','super_admin']);
const ACCOUNT_TYPES=new Set(['employee','developer','contractor','service']);
const LANGUAGES=new Set(['en','th']);
...
async function handleActivate(request,env,origin){
  ...
  try{
    ...
    return json({ok:true,bootstrap:{tokenHash,type:'email'},preferredLanguage:claim.preferred_language==='en'?'en':'th',purpose:claim.purpose},200,origin,env);
  }catch(error){
...
async function handleMicrosoftLink(request,env,origin){
  ...
  const client=supabaseAdmin(env);
  const {data,error}=await client.auth.getUser(token);const user=data?.user;
  ...
  await client.rpc('sindhorn_record_admin_audit',{p_actor_user_id:user.id,p_action:'identity_link',p_entity_type:'sindhorn_employee_identities',p_entity_id:employee.id,p_metadata:{login_method:'microsoft365',email:azure.email}}).catch(()=>{});
  return json({ok:true,profile:employee,loginMethod:'microsoft365'},200,origin,env);
}

function validatedEmployeeInput(body,{existing=null}={}){
  const employeeNumber=normalizeEmployeeNumber(body.employeeNumber??existing?.employee_number);
  const displayName=safeText(body.displayName??existing?.display_name,160);
  const rawEmail=body.workEmail??existing?.work_email??'';const workEmail=normalizeEmail(rawEmail);
  const role=safeText(body.role??existing?.role,32);
  const accountType=safeText(body.accountType??existing?.account_type,32);
  const preferredLanguage=safeText(body.preferredLanguage??existing?.preferred_language,8).toLowerCase();
  const departmentId=(body.departmentId===undefined?existing?.department_id:body.departmentId)||null;
  const active=body.active===undefined?(existing?.active!==false):body.active===true;
  if(!validEmployeeNumber(employeeNumber))throw new Error('invalid_employee_number');
  if(workEmail&&!validEmail(workEmail))throw new Error('invalid_work_email');
  if(!ROLES.has(role))throw new Error('invalid_role');
  if(!ACCOUNT_TYPES.has(accountType))throw new Error('invalid_account_type');
  if(!LANGUAGES.has(preferredLanguage))throw new Error('invalid_preferred_language');
  if(departmentId!==null&&!/^[0-9a-f-]{36}$/i.test(String(departmentId)))throw new Error('invalid_department');
  return{employeeNumber,displayName,workEmail:workEmail||null,departmentId,role,active,preferredLanguage,accountType};
}
function canManageTarget(actor,targetRole,nextRole,nextAccountType){
  if(actor.role==='super_admin')return true;
  return actor.role==='admin'&&!['admin','super_admin'].includes(targetRole||'employee')&&!['admin','super_admin'].includes(nextRole)&&nextAccountType!=='developer';
}

async function handleAdminUsers(request,env,origin){
  const client=supabaseAdmin(env),admin=await adminIdentity(client,request,{requireAal2:false,superOnly:false});
  if(!admin)return json({ok:false,error:'admin_access_required'},403,origin,env);
  const {data,error}=await client.from('sindhorn_employees').select('id,employee_number,display_name,work_email,account_type,department_id,role,active,preferred_language,activated_at,deactivated_at,created_at,updated_at,sindhorn_employee_identities(login_method,provider,email,last_used_at)').order('employee_number',{ascending:true});
  if(error)return json({ok:false,error:'users_unavailable'},503,origin,env);
  return json({ok:true,users:data||[],actor:{id:admin.employee.id,role:admin.employee.role}},200,origin,env);
}

async function handleAdminUserCreate(request,env,origin){
  const client=supabaseAdmin(env),admin=await adminIdentity(client,request,{requireAal2:true,superOnly:false});
  if(!admin)return json({ok:false,error:'mfa_or_admin_required'},403,origin,env);
  let body;try{body=await requestJson(request)}catch(error){return json({ok:false,error:String(error.message||error)},400,origin,env)}
  let input;try{input=validatedEmployeeInput(body)}catch(error){return json({ok:false,error:String(error.message||error)},400,origin,env)}
  if(!canManageTarget(admin.employee,'employee',input.role,input.accountType))return json({ok:false,error:'insufficient_role'},403,origin,env);
  const {data:id,error}=await client.rpc('sindhorn_admin_create_employee',{p_actor_user_id:admin.user.id,p_employee_number:input.employeeNumber,p_display_name:input.displayName,p_work_email:input.workEmail,p_department_id:input.departmentId,p_role:input.role,p_active:input.active,p_preferred_language:input.preferredLanguage,p_account_type:input.accountType});
  if(error)return json({ok:false,error:error.code==='23505'?'employee_already_exists':'employee_create_failed'},error.code==='23505'?409:500,origin,env);
  const {data:employee}=await client.from('sindhorn_employees').select('*').eq('id',id).maybeSingle();
  return json({ok:true,employee},201,origin,env);
}

async function handleAdminUserUpdate(request,env,origin){
  const client=supabaseAdmin(env),admin=await adminIdentity(client,request,{requireAal2:true,superOnly:false});
  if(!admin)return json({ok:false,error:'mfa_or_admin_required'},403,origin,env);
  let body;try{body=await requestJson(request)}catch(error){return json({ok:false,error:String(error.message||error)},400,origin,env)}
  const employeeId=safeText(body.employeeId,64);if(!/^[0-9a-f-]{36}$/i.test(employeeId))return json({ok:false,error:'invalid_employee_id'},400,origin,env);
  const {data:existing,error:existingError}=await client.from('sindhorn_employees').select('*').eq('id',employeeId).maybeSingle();
  if(existingError||!existing)return json({ok:false,error:'employee_not_found'},404,origin,env);
  let input;try{input=validatedEmployeeInput(body,{existing})}catch(error){return json({ok:false,error:String(error.message||error)},400,origin,env)}
  if(!canManageTarget(admin.employee,existing.role,input.role,input.accountType))return json({ok:false,error:'insufficient_role'},403,origin,env);
  if(employeeId===admin.employee.id&&(!input.active||input.role!==admin.employee.role))return json({ok:false,error:'cannot_remove_own_admin_access'},409,origin,env);
  const {data:updated,error}=await client.rpc('sindhorn_admin_update_employee',{p_actor_user_id:admin.user.id,p_employee_id:employeeId,p_employee_number:input.employeeNumber,p_display_name:input.displayName,p_work_email:input.workEmail,p_department_id:input.departmentId,p_role:input.role,p_active:input.active,p_preferred_language:input.preferredLanguage,p_account_type:input.accountType});
  if(error)return json({ok:false,error:error.code==='23505'?'employee_already_exists':'employee_update_failed'},error.code==='23505'?409:500,origin,env);
  if(updated!==true)return json({ok:false,error:'employee_not_found'},404,origin,env);
  const {data:employee}=await client.from('sindhorn_employees').select('*').eq('id',employeeId).maybeSingle();
  return json({ok:true,employee},200,origin,env);
}
...
async function handleIssueCode(request,env,origin){
  ...
  const {data:employee,error}=await client.from('sindhorn_employees').select('id,auth_user_id,active,preferred_language,role,account_type').eq('employee_number',employeeNumber).eq('active',true).maybeSingle();
  if(error||!employee)return json({ok:false,error:'employee_not_found'},404,origin,env);
  if(!canManageTarget(admin.employee,employee.role,employee.role,employee.account_type))return json({ok:false,error:'insufficient_role'},403,origin,env);
  ...
  const {data:activationId,error:issueError}=await client.rpc('sindhorn_issue_activation_code',{p_employee_id:employee.id,p_code_hash:codeHash,p_expires_at:expiresAt,p_purpose:purpose,p_actor_user_id:admin.user.id});
  if(issueError||!activationId)return json({ok:false,error:'activation_code_issue_failed'},500,origin,env);
  await client.rpc('sindhorn_record_admin_audit',{p_actor_user_id:admin.user.id,p_action:'activation_code_issued',p_entity_type:'sindhorn_employees',p_entity_id:employee.id,p_metadata:{purpose,expires_at:expiresAt}}).catch(()=>{});
...
async function handle(request,env){
  ...
  if(url.pathname==='/admin/users'&&request.method==='GET')return handleAdminUsers(request,env,origin);
  if(url.pathname==='/admin/users'&&request.method==='POST')return handleAdminUserCreate(request,env,origin);
  if(url.pathname==='/admin/users/update'&&request.method==='POST')return handleAdminUserUpdate(request,env,origin);
