...
/* ---------------------------------------------------------------------------
   Periods are rows in flipgazine_periods, exactly like books are rows in
   flipgazine_catalog: adding or recolouring one is an insert or an update, with
   no deploy. The array above is a first-paint snapshot only -- the page cannot
   wait on a network round trip before it has a colour. When the fetch lands we
   swap the registry and, if the current period actually changed, crossfade to
   it. Colours are hex in the table because that is what a person edits.
--------------------------------------------------------------------------- */
var FG_ANON_KEY="FG_ANON_KEY_REDACTED";
function hexToRgb(h){
...
fetch("https://sjpvhgxacsiorrtijqua.supabase.co/rest/v1/flipgazine_periods?select=*&published=eq.true&order=sort_order.asc",
      {headers:{apikey:FG_ANON_KEY,Authorization:"Bearer "+FG_ANON_KEY}})
  .then(r=>r.ok?r.json():null)
  .then(rows=>{
    if(!rows||!rows.length) return;                   // keep the snapshot
    const mapped=rows.map(r=>({
      k:r.key, s:r.start_hour, e:r.end_hour, name:r.name, mood:r.mood||"",
      bg:hexToRgb(r.bg), bg2:hexToRgb(r.bg2||r.bg), surface:hexToRgb(r.surface), amb:hexToRgb(r.ambient),
      text:hexToRgb(r.text_color), muted:hexToRgb(r.muted), accent:hexToRgb(r.accent),
      tracks:[r.track_a,r.track_b].filter(Boolean)
    }));
    PERIODS.length=0; mapped.forEach(p=>PERIODS.push(p));
    if(!ED.on||ED.pin) return;
    const p=periodNow();
    if(ED.cur&&ED.cur.k===p.k){ edPaint(p); setPeriodLabel(p); }  // same period, refreshed colours
    else edGoTo(p,900);                                            // snapshot was wrong; ease across
  })
  .catch(()=>{});
