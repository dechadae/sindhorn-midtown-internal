...
const paneDrops=Array.from({length:MAX_PANE_DROPS},(_,i)=>({
  ...
  radius:3.6+seeded(i*17+3)*6.8,aspect:.78+seeded(i*17+4)*.46,asym:(seeded(i*17+5)-.5)*.34,
  speed:7+seeded(i*17+6)*28,trail:18+seeded(i*17+7)*72,phase:seeded(i*17+8)*Math.PI*2,
  alpha:.28+seeded(i*17+9)*.34,drift:(seeded(i*17+10)-.5)*.35,
...
const paneBeads=Array.from({length:MAX_PANE_BEADS},(_,i)=>({
  ...
  radius:.9+seeded(i*13+3)*2.7,aspect:.72+seeded(i*13+4)*.70,rotation:(seeded(i*13+5)-.5)*34,
  alpha:.08+seeded(i*13+6)*.18,phase:seeded(i*13+7)*Math.PI*2,node:null
}));
...
function drawDrop(drop,index,dt,intensity){
  ...
  const x=drop.x*width,y=drop.y*height,drift=lengthPx*(.14+drop.slant*1.9),alpha=(.035+.085*drop.alpha)*intensity;
  ctx.strokeStyle=`rgba(228,238,246,${alpha.toFixed(4)})`;ctx.lineWidth=.45+intensity*.24;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-drift,y+lengthPx);ctx.stroke();
  if(intensity>.62&&index%7===0){ctx.strokeStyle=`rgba(244,248,250,${(alpha*.28).toFixed(4)})`;ctx.lineWidth=.35;ctx.beginPath();ctx.moveTo(x+6,y-lengthPx*.30);ctx.lineTo(x+6-drift*.66,y+lengthPx*.35);ctx.stroke()}
}
...
function buildPaneSvg(){
  ...
  bodyGrad.append(
    makeSvgNode('stop',{offset:'0%','stop-color':'#ffffff','stop-opacity':'.24'}),
    makeSvgNode('stop',{offset:'18%','stop-color':'#eef7fb','stop-opacity':'.09'}),
    makeSvgNode('stop',{offset:'58%','stop-color':'#d8e7ef','stop-opacity':'.025'}),
    makeSvgNode('stop',{offset:'82%','stop-color':'#617481','stop-opacity':'.09'}),
    makeSvgNode('stop',{offset:'100%','stop-color':'#13212b','stop-opacity':'.16'})
  );
  const beadGrad=makeSvgNode('radialGradient',{id:'rainBeadBody',cx:'34%',cy:'26%',r:'74%',fx:'26%',fy:'18%'});
  beadGrad.append(
    makeSvgNode('stop',{offset:'0%','stop-color':'#ffffff','stop-opacity':'.22'}),
    makeSvgNode('stop',{offset:'34%','stop-color':'#eef6fa','stop-opacity':'.055'}),
    makeSvgNode('stop',{offset:'78%','stop-color':'#71838f','stop-opacity':'.065'}),
    makeSvgNode('stop',{offset:'100%','stop-color':'#17242d','stop-opacity':'.11'})
  );
  ...
  trailGrad.append(
    ...
    makeSvgNode('stop',{offset:'72%','stop-color':'#e9f2f6','stop-opacity':'.08'}),
    makeSvgNode('stop',{offset:'100%','stop-color':'#f7fafb','stop-opacity':'.16'})
  );
  ...
  paneDrops.forEach((drop,i)=>{
    ...
    const shade=makeSvgNode('path',{fill:'none',stroke:'rgba(24,35,43,.13)','stroke-linecap':'round'});
    const body=makeSvgNode('path',{fill:'url(#rainDropBody)',stroke:'none'});
    const highlight=makeSvgNode('path',{fill:'none',stroke:'rgba(255,255,255,.30)','stroke-linecap':'round'});
...
function updatePaneDrop(drop,index,dt,intensity,time){
  ...
  const r=drop.radius*drop.growth*(.90+intensity*.16);
  const pulse=moving?1+Math.sin(time*.9+drop.phase)*.035:1;
  const stretch=speedStretch*pulse;
  const trail=moving?drop.trail*(.35+intensity*.58):0;
  const alpha=(.10+.30*drop.alpha)*Math.pow(intensity,.62);
  ...
  drop.node.style.opacity=String(Math.min(.46,alpha));
  ...
  if(trail>4){
    ...
    drop.trailNode.setAttribute('stroke-width',String(Math.max(.45,r*.11)));
    drop.trailNode.style.opacity=String(Math.min(.34,.10+intensity*.24));
  ...
  drop.highlightNode.setAttribute('stroke-width',String(Math.max(.32,r*.055)));
...
function frame(now){
  ...
  if(currentIntensity>.006){const active=Math.round(28+currentIntensity*(MAX_DROPS-28));for(let i=0;i<active;i++)drawDrop(drops[i],i,dt,currentIntensity)}

  const showPane=paneEnabled()&&currentIntensity>.015;
  if(paneSvg)paneSvg.style.opacity=showPane?String(Math.min(.72,.22+currentIntensity*.50)):'0';
  if(showPane){
    const beadActive=Math.round(16+currentIntensity*(MAX_PANE_BEADS-16));
    for(let i=0;i<paneBeads.length;i++)if(paneBeads[i].node)paneBeads[i].node.style.opacity=i<beadActive?String(paneBeads[i].alpha*(.45+currentIntensity*.52)):'0';
