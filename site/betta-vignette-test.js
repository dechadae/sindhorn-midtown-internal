...
titleNode.textContent='Final Reference Tails';
descriptionNode.textContent='Single approved-direction pass only: original radial-membrane engine with each tail independently reframed, reshaped and color-calibrated against its photographic reference.';
...
function paint(){
  ...
  stateNode.textContent=`${day.periodName||day.targetPeriodKey||'—'} · Fish #${current?.referenceId||'—'} · final reference tail · ${Math.round((Number(day.transitionMix)||0)*100)}%`;
...
const {initEnvironment}=await import('./betta-environment.js?v=final-ref-1');
