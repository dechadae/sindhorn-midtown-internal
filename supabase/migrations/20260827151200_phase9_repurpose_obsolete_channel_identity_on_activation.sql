-- Phase 9: manual employee-code activation must preserve the one-Auth-user /
-- one-Sindhorn-identity invariant. If an obsolete SMS/email test identity already
-- owns the reusable Auth user, convert that same identity row to employee_id
-- instead of inserting a duplicate auth_user_id.
