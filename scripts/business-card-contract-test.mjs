...
const [settingsSource,publicSource,publicCss,functionSource,migrationSource]=await Promise.all([
...
for(const required of ['.public-card-panel{','.public-card-head{text-align:center}','.public-card-detail{','.public-card-detail span{'])if(!publicCss.includes(required))throw new Error(`Centered card CSS contract missing ${required}`);
if(!publicCss.includes('text-align:center'))throw new Error('Centered card contract missing');

console.log(JSON.stringify({slug:'dechak',url:`${ORIGIN}/dechak`,vcardUrl:`${ORIGIN}/dechak.vcf`,hotel:HOTEL,logo:LOGO,qr:{version:6,errorCorrection:'M',quietZoneModules:4,viewBox:49},ui:{singleCardDocument:true,settingsActions:'settings-quiet-action',editDialog:'settings-primitives',publicActions:'uniform',alignment:'center'}},null,2));
