...
function applyConfig(next){config=mergeConfig(next);if(uniforms){uniforms.uCloudContrast.value=clamp(Number(config.clouds.contrast)||1,.4,2.4);uniforms.uCloudOpacity.value=clamp(Number(config.clouds.opacity)||1,0,1.5);uniforms.uCloudBroadScale.value=clamp(Number(config.clouds.broadScale)||1,.35,1.8);uniforms.uCloudDetailScale.value=clamp(Number(config.clouds.detailScale)||1,.5,2.4);uniforms.uCloudOvercastFloor.value=clamp(Number(config.clouds.overcastFloor)||.24,0,.8);uniforms.uCloudEdgeLight.value=clamp(Number(config.clouds.edgeLight)||.22,0,.8);uniforms.uStormScale.value=clamp(Number(config.storm.darkening)||.72,0,1.2);uniforms.uFogScale.value=clamp(Number(config.fog.strength)||1,0,2);uniforms.uPmScale.value=clamp(Number(config.pm25.hazeStrength)||1,0,2);uniforms.uTiltScale.value.set(clamp(Number(config.tilt.x)||.022,0,.06),clamp(Number(config.tilt.y)||.016,0,.06));requestRender()}}
...
function buildRenderer(){
  ...
  uniforms={
    ...
    uTemperature:{value:30},uHumidity:{value:.72},uVisibility:{value:20},uCloudCover:{value:.2},uStorm:{value:0},uFog:{value:0},uHeat:{value:0},uWind:{value:8},uWindDirection:{value:225},uPm25:{value:12},uDust:{value:0},uSmoke:{value:0},uFlash:{value:0},uTilt:{value:new THREE.Vector2(0,0)},uTiltScale:{value:new THREE.Vector2(.022,.016)},uCloudContrast:{value:1},uCloudOpacity:{value:1},uCloudBroadScale:{value:1},uCloudDetailScale:{value:1},uCloudOvercastFloor:{value:.24},uCloudEdgeLight:{value:.22},uStormScale:{value:.72},uFogScale:{value:1},uPmScale:{value:1}
  };
...
varying vec2 vUv;
uniform float uTime,uSolarAltitude,uMoonAltitude,uMoonIllumination,uMoonPhase,uTemperature,uHumidity,uVisibility,uCloudCover,uStorm,uFog,uHeat,uWind,uWindDirection,uPm25,uDust,uSmoke,uFlash,uCloudContrast,uCloudOpacity,uCloudBroadScale,uCloudDetailScale,uCloudOvercastFloor,uCloudEdgeLight,uStormScale,uFogScale,uPmScale;
...
void main(){
  ...
  float broad=fbm((p+drift*.28)*vec2(.92,.58)*uCloudBroadScale);float lobe=fbm((p+drift*.48)*vec2(1.72,1.02)*uCloudBroadScale+vec2(broad*.72,7.2));float detail=fbm((p+drift*.86)*vec2(3.6,2.35)*uCloudDetailScale+vec2(lobe*.52,19.0));float field=broad*.58+lobe*.30+detail*.12;float threshold=mix(.78,.43,uCloudCover);float edge=.11/max(.72,uCloudContrast);float cloud=smoothstep(threshold-edge,threshold+edge,field);float over=smoothstep(.84,1.0,uCloudCover);float overMass=smoothstep(.38,.68,broad*.64+lobe*.36);cloud=max(cloud,over*uCloudOvercastFloor*overMass);float crown=smoothstep(.48,.78,field+.055*detail);float underside=smoothstep(.72,.38,field-.08*uv.y);float rim=smoothstep(.08,.34,cloud)*(1.0-smoothstep(.60,.96,cloud));float night=1.0-smoothstep(-7.0,8.0,uSolarAltitude);vec3 cloudBase=vec3(.61,.66,.72);vec3 cloudTop=vec3(.97,.985,1.0);vec3 cloudDay=mix(cloudBase,cloudTop,sat(crown*.82+rim*uCloudEdgeLight));cloudDay=mix(cloudDay,vec3(.54,.59,.66),underside*.20);vec3 cloudNight=mix(vec3(.08,.095,.14),vec3(.28,.32,.39),crown);vec3 cc=mix(cloudDay,cloudNight,night);cc=mix(cc,vec3(.08,.09,.12),uStorm*uStormScale);float cloudAlpha=mix(.28,.90,uCloudCover)*uCloudOpacity;c=mix(c,cc,cloud*clamp(cloudAlpha,0.0,1.0));
  vec2 sd=uv-uSun;sd.x*=aspect;float sunDist=length(sd);float sunAbove=smoothstep(-4.0,1.0,uSolarAltitude);float cloudOptical=sat(cloud*clamp(cloudAlpha,0.0,1.0));float sunTransmission=mix(1.0,.14,cloudOptical);float sunDiffuse=mix(1.0,.55,cloudOptical);float sunCoreGlow=exp(-sunDist*sunDist*1800.0);float sunHalo=exp(-sunDist*sunDist*180.0);float sunAura=exp(-sunDist*sunDist*38.0);vec3 sunWarm=mix(vec3(1.0,.68,.36),vec3(1.0,.98,.86),smoothstep(5.0,40.0,uSolarAltitude));c+=sunWarm*(sunCoreGlow*.74*sunTransmission+sunHalo*.22*sunDiffuse+sunAura*.055*sunDiffuse)*sunAbove;
  vec2 md=uv-uMoon;md.x*=aspect;float moonDist=length(md);float moonR=.020;vec2 moonNormal=md/moonR;float moonRadius2=dot(moonNormal,moonNormal);float moonSurfaceZ=sqrt(max(0.0,1.0-moonRadius2));float moonLimb=1.0-smoothstep(.91,1.0,sqrt(moonRadius2));float moonPhaseAngle=uMoonPhase*6.28318530718;vec2 moonLightDirection=vec2(sin(moonPhaseAngle),-cos(moonPhaseAngle));float moonTerminator=smoothstep(-.075,.075,dot(vec2(moonNormal.x,moonSurfaceZ),moonLightDirection));float moonAbove=smoothstep(-4.0,1.0,uMoonAltitude);float moonCloudTransmission=mix(1.0,.06,cloudOptical);float moonNight=1.0-smoothstep(-7.0,8.0,uSolarAltitude);float moonVisibility=moonAbove*moonCloudTransmission*(1.0-uStorm*.68)*(1.0-uFog*.76);float moonEarthshine=mix(.004,.026,moonNight)*(1.0-moonTerminator);float moonLimbShade=pow(moonSurfaceZ,.24);float moonPhaseBrightness=mix(.68,1.0,uMoonIllumination);float moonDayBrightness=mix(.20,1.0,moonNight);float moonDisc=moonLimb*(moonEarthshine+moonTerminator*moonLimbShade*moonPhaseBrightness)*moonVisibility*moonDayBrightness;float moonHalo=exp(-moonDist*moonDist*420.0)*(1.0-moonLimb)*(.014+.038*uMoonIllumination)*moonVisibility*mix(.45,1.0,moonNight);vec3 moonCool=vec3(.82,.90,1.0);c+=moonCool*(moonDisc*.92+moonHalo);
...
function updateDebug(){if(!new URLSearchParams(location.search).has('debug'))return;if(!debug){debug=document.createElement('div');debug.className='environment-debug';document.body.appendChild(debug)}const v=state.visual,w=state.weather,loc=observerLocation();debug.textContent=['ATMOSPHERE TESTER CORE · LIVE',`location ${loc.latitude.toFixed(3)}, ${loc.longitude.toFixed(3)} · ${loc.source}`,`sun ${state.solar?.altitude?.toFixed?.(1)??'—'}° / moon ${state.lunar?.altitude?.toFixed?.(1)??'—'}°`,`PM2.5 ${state.air.pm??'—'} / AQI ${state.air.aqi??'—'}`,`weather ${w.known?(w.cached?'CACHED':'LIVE'):'pending'} · ${v?.type??'—'}`,v?`cloud ${Math.round(v.cloud*100)}% · rain ${Math.round(v.rain*100)}% · snow ${Math.round(v.snow*100)}% · storm ${Math.round(v.storm*100)}%`:'visual pending',`visibility ${w.visibilityKm?.toFixed?.(1)??'—'} km · wind ${Math.round(w.windSpeedKmh||0)} km/h`,`renderer DPR ${DPR.toFixed(2)} · tester core promoted`].join('\n')}
...
export async function initEnvironment(){
  ...
  document.addEventListener('sindhorn:air-updated',()=>{syncState();requestRender()});document.addEventListener('sindhorn:route-mounted',renderWeather);document.addEventListener('sindhorn:environment-config',event=>applyConfig(event.detail));document.addEventListener('sindhorn:location-updated',()=>{syncState();fetchWeather().catch(()=>{}).finally(()=>{syncState();renderWeather();requestRender()})});
  window.SindhornEnvironment={refreshWeather:()=>fetchWeather().catch(()=>state.weather),renderExport,getState:()=>({weather:{...state.weather,visual:{...state.visual}},air:{...state.air},solar:{...state.solar},lunar:{...state.lunar},location:observerLocation(),quality:DPR,config:structuredClone(config),renderer:'atmosphere-tester-core'}),applyConfig};
