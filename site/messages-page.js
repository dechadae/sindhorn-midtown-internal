...
function alertRow(message) {
  ...
  return `<button class="app-list-row" type="button" data-message-open="${esc(hashFor(message.route))}">
    <span class="app-list-row-main"><span class="app-list-row-title">${esc(title)}</span><span class="app-list-row-meta">${esc(kindLabel(message.kind))} · ${esc(formatDateTime(message.receivedAt))}</span>${body ? `<span class="app-list-row-copy">${esc(body)}</span>` : ''}</span>
...
function markup(broadcasts, alerts, inbox) {
  ...
  const note = inbox.cached && inbox.at ? `Saved on this phone · ${esc(formatDateTime(inbox.at))}` : inbox.cached ? 'Offline · broadcasts will load when the connection returns' : '';
  if (!rows.length) return `${hero}<section class="app-section"><div class="app-stack"><div class="app-state app-card" data-tone="${inbox.cached ? 'error' : 'empty'}"><p class="app-state-label">${inbox.cached ? 'Offline' : 'Empty'}</p><p class="app-state-title">${inbox.cached ? 'Couldn\'t load messages' : 'No messages yet'}</p><p class="app-state-copy">${inbox.cached ? 'Check the connection and pull to refresh.' : 'Broadcasts from the hotel and alerts sent to this device will appear here.'}</p></div></div></section>`;
