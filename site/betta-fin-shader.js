...
float noiseField(float u,float v,float t,float seed){
  vec3 p=vec3(u*1.75+seed*.17,v*2.9-seed*.11,t*.095+seed);
  float n=snoise(p);
  n+=.52*snoise(p*1.91+vec3(7.1,-3.4,t*.021));
  n+=.24*snoise(p*3.73+vec3(-4.2,9.8,-t*.017));
  return n/1.76;
}
vec3 membranePosition(float u,float v,float jitter){
  float t=uTime*uMotionSpeed+uPhase;
  ...
  float currentBend=(uCurrent.x*cos(theta)+uCurrent.y*sin(theta))*uCurrentStrength;
  float fold=sin((v*uFoldDensity+secondary*.12+slow*.08)*6.28318+uSeed)*(.26+.5*u);
  float curlAngle=uCurl*u*u*(.52+slow*.24)+currentBend*u*.13;
  theta+=curlAngle+broad*.08*uMotionAmplitude*rootEase;
  float twist=uTwist*u*u+slow*.22*uTurbulence;
  vec3 p=vec3(cos(theta),sin(theta),0.0)*r;
  float thickness=sin(theta*1.7+twist)*uDepth*u*.22;
  p.z+=thickness+fold*uDepth*.28*rootEase;
  p.z+=(broad*.34+secondary*.13)*uMotionAmplitude*rootEase*(.4+.6*u);
  p.xy+=uCurrent*uCurrentStrength*u*u*.32;
  float flutter=noiseField(u*5.4,v*8.5,t*2.3,uSeed+31.0);
  p.z+=flutter*uEdgeFlutter*tipEase*tipEase*.17;
  p.xy+=vec2(-sin(theta),cos(theta))*flutter*uEdgeFlutter*tipEase*.055;
...
void main(){
  ...
  float gradient=clamp(vFinUv.x*.67+vFinUv.y*.28+uGradientPosition+.055*sin(vFinUv.y*13.0+uSeed),0.0,1.0);
  vec3 base=palette(gradient);
  float irShift=(fresnel*.65+vFold*.35)*uIridescence;
  base=mix(base,base.brg,irShift*.28);
  base=saturateColor(base,uSaturation)*uBrightness;
