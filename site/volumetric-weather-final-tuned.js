const sourceUrl=new URL('./volumetric-weather-final.js',import.meta.url);sourceUrl.searchParams.set('tune','broad-banks-wet-glass-v5');const seasonalUrl=new URL('./seasonal-sky.js',import.meta.url).href;let source=await fetch(sourceUrl,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`base renderer ${r.status}`);return r.text()});source=source.replace("from './seasonal-sky.js'",`from '${seasonalUrl}'`);
const densityStart=source.indexOf('float densityAt(vec3 p,float skyY){');const densityEnd=source.indexOf('\nvoid main(){',densityStart);if(densityStart<0||densityEnd<0)throw new Error('v5 density contract missing');const density=`float densityAt(vec3 p,vec2 screenUv){
 ...
 vec3 q=vec3(p.x*.058+uWind.x,p.z*.052+uWind.y,h*.52+uTime*.00012);
 ...
 float broad=n0*.60+n1*.28+n2*.12;
 float w0=texture(uWeatherTex,vec2(screenUv.x*.11,screenUv.y*.30)+uWind*.22).r;
 float w1=texture(uWeatherTex,vec2(screenUv.x*.22+.21,screenUv.y*.60+.13)+uWind*.13).r;
 float bank=w0*.72+w1*.28;
 float bankThreshold=mix(.68,.38,uCoverage)-uConnected*.12;
 float bankGate=smoothstep(bankThreshold-.08,bankThreshold+.07,bank);
 bankGate=mix(bankGate,1.,sat(uConnected*.88));
 float threshold=mix(.70,.39,uCoverage)-uConnected*.07;
 float field=broad+(bank-.5)*.28+uConnected*.08;
 float edge=1.-smoothstep(.055,.20,abs(field-threshold));float er=texture(uErosionNoise,q*4.8+vec3(.19,.41,.11)).r-.5;
 field-=er*uErosion*.18*edge*(.58+.42*h);
 float d=smoothstep(threshold-.07,threshold+.055,field)*vertical*uDensity*bankGate;
 d*=mix(.74,1.15,broad);d*=mix(.98,1.06,screenUv.y);return d*sat(uCoverage*1.18);
}`;source=source.slice(0,densityStart)+density+source.slice(densityEnd);source=source.replace('float d=densityAt(p,vUv.y);','float d=densityAt(p,vUv);').replace("vec3 sq=vec3((p.x+uSunDir.x*.65)*.020+uWind.x,(p.z+uSunDir.z*.65)*.037+uWind.y,h*.52+uSunDir.y*.10);","vec3 sq=vec3((p.x+uSunDir.x*.65)*.058+uWind.x,(p.z+uSunDir.z*.65)*.052+uWind.y,h*.52+uSunDir.y*.10);");
const wetStart=source.indexOf('function updateWetness(dt){');const wetEnd=source.indexOf('\nupdateWetness(16);',wetStart);if(wetStart<0||wetEnd<0)throw new Error('v5 wet-glass contract missing');const wet=`function updateWetness(dt){
