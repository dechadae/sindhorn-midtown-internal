...
/* Sky mode (r30) draws the period's own four colors as slow plumes over its
   ...
   for a week; automation (navigator.webdriver) is never judged. Plume
...
const SKY_MEMORY_KEY='sindhorn-midtown:betta-mode:v1',SKY_MEMORY_MS=7*24*60*60*1000,SKY_PLUME_ENERGY=.42,SKY_PLUME_RANGE=[.12,.6],SKY_DRIFT_SECONDS=90,LOW_END_FRAME_MS=28,GOVERNOR_WARMUP_MS=600,GOVERNOR_WINDOW_MS=3000;
...
function resolveMode(){if(mode.requested!=='auto')return[mode.requested,'requested'];if(reducedMotionQuery.matches)return['sky','reduced-motion'];if(rememberedLowEnd())return['sky','low-end'];return['betta','default']}
...
function stepGovernor(now,delta){const g=mode.governor;if(g.done||mode.current!=='betta'||mode.requested!=='auto'||navigator.webdriver)return;if(!g.startedAt){g.startedAt=now;return}if(now-g.startedAt<GOVERNOR_WARMUP_MS)return;g.frames.push(delta);if(now-g.startedAt<GOVERNOR_WARMUP_MS+GOVERNOR_WINDOW_MS)return;g.done=true;const sorted=[...g.frames].sort((a,b)=>a-b);mode.frameMs=sorted[sorted.length>>1]||0;if(mode.frameMs>LOW_END_FRAME_MS){rememberLowEnd();applyMode('sky','low-end')}}
