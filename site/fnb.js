...
export async function mountFnbRoute(root,{profile}={}){
  ...
  function statBlock(label,value){return`<div class="fnb-stat"><b>${esc(value)}</b><span>${esc(label)}</span></div>`}
  ...
  function activationHTML(activation){const total=activation.artworks.length,done=activation.artworks.filter(item=>isDone(item.id)).length,complete=total>0&&done===total;return`<article class="fnb-art-card${complete?' is-complete':''}"><div class="fnb-art-head"><div><strong>${esc(activation.outlet)}</strong><span class="fnb-art-meta">${esc(activation.time)} · IHG One Rewards ${esc(activation.discount)}</span></div><span class="fnb-art-tally">${done}/${total}<i>${complete?'✓':''}</i></span></div><div class="fnb-task-list">${taskRows(activation)}</div></article>`}
