...
function clearLegacyLocalLinks(){
  try{if(localStorage.getItem(LINK_MIGRATION_KEY)==='1')return;const local=safeParse(localStorage.getItem(LOCAL_STATE_KEY)||'')||{};localStorage.setItem(LOCAL_STATE_KEY,JSON.stringify({checks:local.checks||{},links:{}}));localStorage.setItem(LINK_MIGRATION_KEY,'1')}catch(_){}
}
...
function decorate(data){
  return data.map(p=>{
    ...
    const present=new Set(activations.map(a=>a.outlet));const reference=activations[0]||{};
    for(const outlet of display){if(!present.has(outlet))activations.push({id:`__display__${p.id}__${displaySlug(outlet)}`,outlet,time:reference.time||'TBC',discount:reference.discount||'N/A',brief:'',copyEn:'',copyTh:'',artworkUrl:null,artworks:[],displayOnly:true})}
    return {...p,displayOutlets:display,activations}
  })
}
...
function ensureAdapterStyle(){if(document.getElementById('fnb-data-adapter-style'))return;const style=document.createElement('style');style.id='fnb-data-adapter-style';style.textContent='.fnb-art-card[data-activation^="__display__"]{display:none!important}.fnb-route [data-folder-edit]{display:none!important}.fnb-data-note{margin:10px 0 0;font-size:12px;line-height:1.35;opacity:.72}';document.head.appendChild(style)}
function optionMarkup(value){const label=value;return `<button class="fnb-select-option" type="button" role="option" aria-selected="false" data-filter-option="outlet" data-value="${label.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"><span>${label}</span><i aria-hidden="true"></i></button>`}
function patchOutletFilter(){const route=document.querySelector('.fnb-route');if(!route)return;const field=route.querySelector('[data-filter-field="outlet"]'),select=field?.querySelector('[data-outlet-select]'),menu=field?.querySelector('.fnb-select-menu');if(!field||!select||!menu)return;for(const outlet of EXTRA_OUTLETS){if(!select.querySelector(`option[value="${CSS.escape(outlet)}"]`)){const opt=document.createElement('option');opt.value=outlet;opt.textContent=outlet;select.appendChild(opt)}if(!menu.querySelector(`[data-value="${CSS.escape(outlet)}"]`))menu.insertAdjacentHTML('beforeend',optionMarkup(outlet))}}
function patchStatus(){const route=document.querySelector('.fnb-route');if(!route)return;route.querySelector('.fnb-data-note')?.remove();if(source==='supabase'||source==='supabase-public')return;const hero=route.querySelector('.fnb-hero');if(!hero)return;const note=document.createElement('div');note.className='fnb-data-note';note.textContent=source==='cache'?'Offline · showing last saved F&B data':'Offline · limited F&B fallback';hero.appendChild(note)}
function patchUpdated(){const route=document.querySelector('.fnb-route'),title=route?.querySelector('.fnb-detail-title')?.textContent?.trim();if(!route||!title)return;const item=FNB_PROMOTIONS.find(p=>p.title===title),updated=item?.updatedAt;if(!updated)return;for(const fact of route.querySelectorAll('.fnb-fact')){if(fact.querySelector('span')?.textContent?.trim()==='Updated'){const b=fact.querySelector('b');if(b){const d=new Date(updated);b.textContent=Number.isNaN(d.valueOf())?String(updated).slice(0,10):new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Bangkok',day:'numeric',month:'long',year:'numeric'}).format(d)}}}}
function enhance(){ensureAdapterStyle();patchOutletFilter();patchStatus();patchUpdated()}
document.addEventListener('sindhorn:route-mounted',event=>{if(event.detail?.route==='fnb')queueMicrotask(enhance)});
if(typeof MutationObserver!=='undefined'){const observer=new MutationObserver(()=>{if(document.body?.dataset.route==='fnb')queueMicrotask(enhance)});if(document.documentElement)observer.observe(document.documentElement,{childList:true,subtree:true})}
