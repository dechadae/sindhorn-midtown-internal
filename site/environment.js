...
function renderWeather(){
  ...
  set('weatherTemp',`${Math.round(w.temperatureC)}°`);set('weatherConditionEn',en);set('weatherConditionTh',th);set('weatherMetaEn',`Feels ${feels}° · RH ${Math.round(w.humidity*100)}% · Wind ${windPoint(w.windDirectionDeg)} ${Math.round(w.windSpeedKmh)} km/h`);set('weatherMetaTh',`รู้สึกเหมือน ${feels}° · ความชื้น ${Math.round(w.humidity*100)}% · ลม ${windPoint(w.windDirectionDeg)} ${Math.round(w.windSpeedKmh)} กม./ชม.`);weatherNow.hidden=false;
}
