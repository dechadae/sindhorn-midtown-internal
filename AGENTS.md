...
- **Job card invariant (r23): a job card's groups (the ask, the facts, the actions) are separated by `.app-job-section` hairlines; the status is `.app-badge.app-job-status`, a button that opens the status sheet (`.app-sheet` on the page root with `button.app-list-row[aria-pressed]` rows). A menu never opens inside a card - it would nest glass.**
