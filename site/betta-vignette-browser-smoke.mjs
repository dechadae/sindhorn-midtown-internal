...
async function review({period,viewport,grade='selected',suffix}){
  const context=await browser.newContext({viewport,screen:viewport,serviceWorkers:'block'});
  const page=await context.newPage();const errors=[];
  page.on('pageerror',error=>errors.push(String(error)));
  page.on('console',message=>{if(message.type()==='error')errors.push(message.text())});
  await page.goto(`${base}/betta-vignette-test.html?grade=${grade}&period=${period.key}`,{waitUntil:'domcontentloaded',timeout:60000});
  ...
  const result=await page.evaluate(()=>{const state=window.SindhornEnvironment.getState(),canvas=document.querySelector('#environmentCanvas');return{renderer:state.renderer,inputMode:state.inputMode,baselineAuthority:state.betta?.baselineAuthority,period:state.betta?.dayCycle?.targetPeriodKey,baseline:state.betta?.dayCycle?.targetBaseline,referenceId:Number(document.body.dataset.referenceId||0),grade:document.querySelector('[data-grade].is-active')?.dataset.grade,canvas:{width:canvas?.width||0,height:canvas?.height||0,cssWidth:canvas?.clientWidth||0,cssHeight:canvas?.clientHeight||0},viewport:{width:innerWidth,height:innerHeight}}});
  ...
  if(result.grade!==grade)throw new Error(`${period.key}: grade mismatch`);
...
await browser.close();
console.log('Reference-tail reviewer passed all eight mobile fish plus representative desktop views on the original radial engine.');
